...
- **Control Background Jobs** (toggle) — when on, a **Background jobs** manager
  lists every background job the session can see, with a **Terminate** button
  per live job and a **Terminate all running** action. Terminating a job stops
  it so the model stops waiting on it. The list is live (polls while open).
